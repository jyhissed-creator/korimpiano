...
def generar_audio_acorde(notas):
    # Traducimos las notas para Tone.js (Soporta sostenidos con #)
...
# --- INTERFAZ PRINCIPAL ---
st.title("🎹 KORYMpiano")
st.markdown("### The Chord Logic Engine")
st.write("Analiza acordes, visualiza manos y escucha la armonía.")
        ...
        if raiz in NOTAS:
            ...
            with col2:
                ...
                for i, n in enumerate(notas_acorde):
                    ...
                    st.write(f"**{n}** {'⚡' if ints[i] > 12 else ''}")

            # Teclado Visual
            st.markdown("#### Vista del Teclado")
            teclas_html = "<div style='background-color: #222; padding: 20px; border-radius: 10px; text-align: center;'>"
            for n in NOTAS:
                color = "#FF4B4B" if n in notas_acorde else "white"
...
st.sidebar.markdown("---")
st.sidebar.write("### 👤 Creador")
st.sidebar.info("Desarrollado por **Yhissed Jiménez**.")
st.sidebar.write("© 2024 KORYMpiano")
st.sidebar.markdown("### ☕ Apoya el proyecto")
st.sidebar.link_button("Invítame a un café", "https://www.buymeacoffee.com/tu_usuario")
